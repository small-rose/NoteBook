...
XXX_CONSTRAINTS.CONSTRAINT_TYPE :  P 主键, R 外键
...
```sql
SELECT x.WEB_URI, x.* FROM(
    --根据接口进行二次分组，按访问量到排序
SELECT h.*,row_number() over(PARTITION BY WEB_URI order by cnt desc) as row_num FROM (
    -- 查每天，每个接口，累计访问量
SELECT TRUNC (L.CREATETIME)AS CREATETIME, L.WEB_URI WEB_URI, SUM(L.REQUEST_COUNT) AS Cnt FROM AMS_DRUID_URI_LOG L
WHERE L.DATA_SOURCE= 'WS'
GROUP by TRUNC(L.CREATETIME), L.WEB_URI
)h
    -- 取出最大访问量
)x WHERE X.row_num =1;
```